...
GDB ?= ~/Documents/projects/rust-os-gdb/bin/gdb --silent
...
GO_GDB ?= $(GOROOT)/src/runtime/runtime-gdb.py
...
run-qemu: GC_FLAGS += -B
run-qemu: iso
	$(QEMU) -s -S -cdrom $(iso_target) -vga std -d int,cpu_reset -no-reboot
...
gdb: iso
	...
	sleep 1
	$(GDB) -ex 'add-auto-load-safe-path $(shell pwd)'
	@killall $(QEMU) || true
